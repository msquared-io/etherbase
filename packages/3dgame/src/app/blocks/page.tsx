...
function VoxelBlocks({
  ...
  onMeshCreated: (mesh: THREE.Mesh, key: string) => void
  onMeshDestroyed: (key: string) => void
}) {
  ...
  // Track which blocks we're currently rendering
  const renderedBlocks = useRef(new Set<string>())

  // Clean up blocks that no longer exist
  useEffect(() => {
    const currentBlockKeys = new Set(blocks.keys())
    const toRemove = Array.from(renderedBlocks.current).filter(
      (key) => !currentBlockKeys.has(key),
    )

    for (const key of toRemove) {
      renderedBlocks.current.delete(key)
      onMeshDestroyed(key)
    }
  }, [blocks, onMeshDestroyed])

  return (
    <group>
      {Array.from(blocks.values()).map((block) => {
        const key = `${block.position.x},${block.position.y},${block.position.z}`
        renderedBlocks.current.add(key)
...
function Scene({
  ...
  const handleBlockDelete = useCallback(
    (position: THREE.Vector3) => {
      // Round coordinates to match the grid
  ...
  const handleMeshCreated = useCallback((mesh: THREE.Mesh, key: string) => {
    blockMeshes.current.push(mesh)
    blockRefs.current.set(key, mesh)
  }, [])

  const handleMeshDestroyed = useCallback((key: string) => {
    const mesh = blockRefs.current.get(key)
    if (mesh) {
      const index = blockMeshes.current.indexOf(mesh)
      if (index !== -1) {
        blockMeshes.current.splice(index, 1)
      }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "KeyY") {
        ...
        if (intersects.length > 0) {
          handleBlockDelete(intersects[0].point)
        }
      }
      // Clear all non-initial blocks with C key
      if (e.code === "KeyC") {
        try {
          // Get all non-initial block keys that need to be removed
          const blocksToDelete = Array.from(blocks.entries())
            .filter(([key]) => !initialBlocks.has(key))
            .map(([key]) => key)

          // Clean up meshes for each block being deleted
          for (const key of blocksToDelete) {
            handleMeshDestroyed(key)
          }

          // Keep only initial blocks locally
          setBlocks(new Map(initialBlocks))

          // Clear all blocks in etherstore
          update({
            blocks: Object.fromEntries(
              blocksToDelete.map((key) => [key, null]),
            ),
          })
        } catch (err) {
  ...
  // Clear meshes when component unmounts
  useEffect(() => {
    return () => {
      blockMeshes.current = []
  ...
  const handlePlayerUpdate = useCallback(
    (rawEvent: EtherbaseEvent) => {
      const event = rawEvent as unknown as PlayerUpdateEvent
  ...
  return (
    <>
      ...
      <VoxelBlocks
        ...
        onMeshDestroyed={handleMeshDestroyed}
      />
